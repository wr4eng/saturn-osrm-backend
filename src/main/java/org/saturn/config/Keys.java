// src/main/java/org/saturn/config/Keys.java
...
public final class Keys {
    ...
    public static final ConfigKey<String> SERVER_STATISTICS = new StringConfigKey(
            "server.statistics",
            List.of(KeyType.CONFIG),
            "https://saturn.pacific-telematika.co.id/analytics/");
    ...
    /**
     * LDAP server URL. For more info, check <a href="https://saturn.pacific-telematika.co.id:1880/ldap/">LDAP config</a>.
     */
    ...
    // =============================================================================
    // OsrmClient OsrmMatchClient Backend - routing configuration keys
    // =============================================================================
    ...
    /**
     * Routing profile: driving, walking, cycling, car, bike, foot
     * OSRM supported profiles depend on loaded .lua file
     */
    public static final ConfigKey<String> ROUTING_PROFILE = new StringConfigKey(
            "routing.profile",
            List.of(KeyType.CONFIG),
            "driving");

    /**
     * Enable snapping GPS points to road network
     * true: Snap to nearest road | false: Use raw coordinates
     */
    public static final ConfigKey<Boolean> ROUTING_SNAP = new BooleanConfigKey(
            "routing.snap",
            List.of(KeyType.CONFIG),
            true);

    /**
     * Routing API timeout in seconds
     * Recommended: 30-60 seconds for large routes
     */
    public static final ConfigKey<Long> ROUTING_TIMEOUT = new LongConfigKey(
            "routing.timeout",
            List.of(KeyType.CONFIG),
            30L);

    /**
     * Enable routing result caching to improve performance
     */
    public static final ConfigKey<Boolean> ROUTING_CACHE = new BooleanConfigKey(
            "routing.cache",
            List.of(KeyType.CONFIG),
            true);
     ...
     */
    public static final ConfigKey<Long> ROUTING_CACHE_TTL = new LongConfigKey(
            "routing.cache.ttl",
            List.of(KeyType.CONFIG),
            3600000L);

    /**
     * Maximum retry attempts for routing API requests
     * Uses exponential backoff: 1s, 2s, 3s...
     */
    public static final ConfigKey<Integer> ROUTING_RETRY = new IntegerConfigKey(
            "routing.retry",
            List.of(KeyType.CONFIG),
            3);

    /**
     * Enable detailed logging for routing operations
     * Useful for debugging route calculation issues
     */
    public static final ConfigKey<Boolean> ROUTING_DEBUG = new BooleanConfigKey(
            "routing.debug",
            List.of(KeyType.CONFIG),
            false);

    // =============================================================================
    // OsrmClient Routing filter 
    // =============================================================================
    /**
     * Maximum plausible speed in km/h for GPS drift detection.
     * Positions implying higher speed between consecutive points are dropped.
     * Default: 150 km/h. Set 0 to disable.
     */
    public static final ConfigKey<Integer> ROUTING_FILTER_MAX_SPEED = new IntegerConfigKey(
        "routing.filter.maxSpeed",
        List.of(KeyType.CONFIG),
        150);

    /**
     * Maximum GPS accuracy radius in meters to accept a position.
     * Positions with accuracy > threshold are dropped (requires device to send accuracy attribute).
     * Default: 50m. Set 0 to disable.
     */
    public static final ConfigKey<Integer> ROUTING_FILTER_MAX_ACCURACY = new IntegerConfigKey(
        "routing.filter.maxAccuracy",
        List.of(KeyType.CONFIG),
        50);
        
    /**
     * Minimum distance in meters between consecutive positions.
     * Positions closer than this to the previous accepted position are merged/skipped.
     * Reduces stationary GPS noise and tight clusters. Default: 10m. Set 0 to disable.
     */
    public static final ConfigKey<Integer> ROUTING_FILTER_MIN_DISTANCE = new IntegerConfigKey(
        "routing.filter.minDistance",
        List.of(KeyType.CONFIG),
        10);
    ...
    // =============================================================================
    /**
     * Enable OSRM map matching mode using /match/v1 (Hidden Markov Model).
     * More accurate than /route/v1 for noisy GPS traces — drops drift points automatically.
     * Requires routing.type=osrm. Falls back to route/v1 if NoMatch or low confidence.
     * Default: false
     */
    public static final ConfigKey<Boolean> ROUTING_MATCH_ENABLED = new BooleanConfigKey(
            "routing.match.enabled",
            List.of(KeyType.CONFIG),
            false);

    /**
     * Minimum confidence threshold for accepting a /match/v1 segment (0.0 - 1.0).
     * Segments below this threshold are skipped; fallback to /route/v1 if all segments fail.
     * From test results: 0.5 recommended. Set lower (0.1) for very noisy traces.
     * Default: 0.5
     */
    public static final ConfigKey<Double> ROUTING_MATCH_MIN_CONFIDENCE = new DoubleConfigKey(
